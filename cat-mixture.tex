...
&\qquad\qquad +\ind{M_{j}= 3}\frac{\exp{\psi_{j\ell}}}{\sum_{\ell^\prime \in \{0, 1, 2\}}\exp{\psi_{j\ell^\prime}}}
\end{align}
...
\toprule
Voter     & \(\zeta_{i1}\) & \(\cdots\) & \(\zeta_{iK}\) &   Office & \(M_i\) & Option $\ell$ &  \(Y_{ij} = \ell\)\\\midrule
...
We then read off the two coefficients (relative to baseline), which will determine the value for \(\mu_{kj\ell}\). 
