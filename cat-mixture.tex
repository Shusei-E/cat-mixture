...
Voters are still endowed by types \(k \in \{1,..., K\}\), but they now they are also part of another, precisely observed grouping for each office \(M_{ij} \in \{1, 2, 3\}\) which indicates one of these cases. Let \(M_{ij} = 3\) denote the contested case, so the voter has three options. Let \(M_{ij} = 2\) denote the case when only the preferred party candidate is in the race, and let \(M_{ij} = 1\) denote the case when only the opposed party candidate is in the race. 
