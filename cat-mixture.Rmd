...
Index individuals by \(i \in \{1:N\}\) and the universe of races excluding the top of the ticket as \(j \in \{1:D\}\).  The data we observe is a length-\(D\) vector of votes \(\by_i\). \(y_{ij}\) is a discrete response value which can be 1 of \(L\) possibilities. 


\paragraph{Likelihood} For now, let's use \(L = 2\) so that each vote \(y_{ij}\) be a _binary_ variable for splitting their ticket or not. \(y_{ij} = 1\) would mean voter \(i\) splitting their ticket in some office \(j\), with reference to a top of the ticket office like the President or Governor. 
...
The benefit of this modeling exercise over that from a naive sample of \(N \times D\) Bernoullis is that we have captured the correlations between variables.\footnote{That dependency can be expressed as \(\mathds{E}(\by) = \sum_{k = 1}^{K} \theta_{k} \bmu_{k}\) and 
...
We can estimate the key parameters \(\bmu\) and \(\bth\) by EM.\footnote{Thanks to Soichiro Yamauchi for deriving this algorithm in the original iteration.} 
